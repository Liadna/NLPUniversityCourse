...
def read_files_to_dict(dir_path):
    ...
    :return: Dictionary of characters and their conversations
    """
    conversations = {}
...
def get_num_of_PERSON_NE(ne_tagging, character):
    """
    Count number of PERSON NE tages for each character
    :param ne_tagging: NE tagging
    :param character: Analyzed character
...
def build_sentiment_graph(nodes, edges):
    """
    Generate a networkX graph from given nodes and edges
...
def create_nodes(characters_dict, topic_dict):
    """
    Set each unique character as a node in the graph.
    :param characters_dict: Dictionary of each character and his conversations
    :param topic_dict: The assignment of a topic of each character
    ...
    for name, size in characters_dict.iteritems():
        ...
        if name != 'Background':
            nodes.append((name, dict(size=size, name=name, topic=topic)))
    return nodes
...
def create_edges(conversations, characters):
    ...
    :param characters: Edges as characters
    :return: Generated edges
    """
    edges = {}
...
def draw_graph(G):
    ...
    node_color = nx.get_node_attributes(G, 'topic')
...
def main():
    ...
    person_count_dict = {}
    topic_dict = {'Spongebob': 'y', 'Patrick': 'y', 'Barnacle Boy': 'b', 'Mermaid Man': 'b',
                  'Mr. Krabs': 'y', 'Larry': 0.75, 'Karen': 'b', 'Squilliam': 'y', 'Sandy': 0.75, 'Squidward': 'y',
                  'Gary': 0.75, 'Pearl': 0.75, 'Plankton': 'b', 'Mrs. Puff': 'b'}
