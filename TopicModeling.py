...
def main():
    ...
    #################################
    # Topic modeling per character conversations
    character = 'Mrs. Puff'
    ...
    ##################################
    # Topic modeling over all conversations
    normalized = preprocess(doc=''.join(conversations.values()), stopwords_characters=characters_names)
    dictionary, doc_term_matrix = get_doc_term_matrix(doc_clean=[normalized.split()])

    ldamodel, topics = get_lda_topics(num_topics=50,
                                      passes=50,
                                      dictionary=dictionary,
                                      doc_term_matrix=doc_term_matrix,
                                      n_top_terms=20)
